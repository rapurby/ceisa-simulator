...
export default function Dashboard() {
  ...
  const cards = [
    { label: 'Total Masuk',   value: stats?.total,    icon: Inbox,        color: '#1565c0' },
    { label: 'Pending Review',value: stats?.pending,  icon: Clock,        color: '#e65100' },
    { label: 'Diterima',      value: stats?.accepted, icon: CheckCircle,  color: '#2e7d32' },
    { label: 'Ditolak',       value: stats?.rejected, icon: XCircle,      color: '#c62828' },
  ]

  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary)' }}>Dashboard CEISA</h1>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
          Monitoring deklarasi masuk dari Cikarang Dry Port
        </p>
      </div>
      ...
      <div style={{ background: 'var(--card)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontWeight: 700, fontSize: 14 }}>Deklarasi Terbaru</span>
        ...
        {recent.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
            Belum ada deklarasi masuk
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['No. Registrasi','Importir','Invoice','Barang','Status','Waktu'].map(h => (
